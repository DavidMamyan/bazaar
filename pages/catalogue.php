...
?>
			<h1>Catalogue :: <?php echo "<strong>" . $category['name']; "</strong>" ?></h1>
			<?php 
				if (!empty($rows)) {
					foreach ($rows as $row) {
					?>
					<div class="catalogue_wrapper">
						<div class="catalogue_wrapper_left">
							...
							<a href="<?php echo $link ?>">
								<img src="<?php echo $objCatalogue->_path.$image ?>" alt="<?php echo Helper::encodeHTML($row['name'], 1) ?>" width="<?php echo $width ?>">
							</a>
						</div>
						<div class="catalogue_wrapper_right">
							...
							<p><?php echo Helper::shortenString(Helper::encodeHTML($row['description'], 1)) ; ?></p>
