...
if ($objForm->isPost('name')) {
	...
	if ($objValid->isValid()) {
		...
		if ($objCatalogue->updateCategory($objValid->_post, $id)) {
	...
	<table cellspacing="0" cellpadding="0" border="0" class="tbl_insert">
		...
		<tr>
			...
			<td><?php echo $objValid->validate('identity') ?> <?php echo $objValid->validate('duplicate_identity') ?>
				<input type="text" name="identity" id="identity" value="<?php echo $objForm->stickyText('identity', $category['identity']) ?>" class="fld">
			</td>
		</tr>	
		...
		<tr>
			...
			<td><?php echo $objValid->validate('meta_title') ?> 
				<input type="text" name="meta_title" id="meta_title" value="<?php echo $objForm->stickyText('meta_title', $category['meta_title']) ?>" class="fld">
			</td>
		</tr>
		...
		<tr>
			...
			<td><?php echo $objValid->validate('meta_description') ?>
				<textarea name="meta_description" id="meta_description" class="tar_fixed"><?php echo $objForm->stickyText('meta_description', $category['meta_description']) ?></textarea>
			</td>
		</tr>
		...
		<tr>
			...
			<td><?php echo $objValid->validate('meta_keywords') ?>
				<textarea name="meta_keywords" id="meta_keywords" class="tar_fixed"><?php echo $objForm->stickyText('meta_keywords', $category['meta_keywords']) ?></textarea>
			</td>
		</tr>
