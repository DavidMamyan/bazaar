		...
		<?php 
			if (!empty($items)) { ?>
				<tr>
					...
					<td>Item</td>
					<td class="col_5 ta_r">Quantity</td>
					<td class="col_15 ta_r">Amount</td>
				</tr>
				<?php foreach ($items as $item): ?>
					<?php $product = $objCatalogue->getProduct($item['product']); ?>
					<tr>
						<td><?php echo $product['id']; ?></td>
						<td><?php echo Helper::encodeHTML($product['name']); ?></td>
						<td class="ta_r"><?php echo $item['qty']; ?></td>
						<td class="ta_r">&#36;<?php echo number_format(($item['price'] * $item['qty']),2) ?></td>
					</tr>
				<?php endforeach ?>
			...
			<tr>
				<th>Sub-total:</th>
				<td colspan="4" class="ta_r">&#36;<?php echo number_format($order['subtotal'], 2); ?></td>
			</tr>
			<tr>
				<th>VAT(<?php echo $order['vat_rate'] ?>)%:</th>
				<td colspan="4" class="ta_r">&#36;<?php echo number_format($order['vat'], 2); ?></td>
			</tr>
			<tr>
				<th><strong>Total:</strong></th>
				<td colspan="4" class="ta_r"><strong>&#36;<?php echo number_format($order['total'], 2); ?></strong></td>
			</tr>
			...
			<tr>
				...
				<td colspan="4">
					<?php 
						echo $objValid->validate("status");
						if (!empty($status)) { ?>
							<select name="status" id="status" class="sel">
								<?php foreach ($status as $stat): ?>
									<option value="<?php echo $stat['id'] ?>">
										<?php $objForm->stickySelect("status", $stat['id'], $order['status']); ?>
			...
			<tr>
				...
				<td colspan="4">
					<div class="sbm sbm_blue fl_r">
						<a href="<?php echo $this->objUrl->getCurrent(array('action')).'/action/invoice'; ?>" class="btn" target="_blank">Invoice</a>
					</div>
