var basketObject = {
    ...
    refreshBigBasket : function() {
        jQuery.post('http://bazaar/mod/basket_view.php', function(data) {
    ...
    removeFromBasket : function(obj) {
        obj.live('click', function() {
            ...
            jQuery.post('http://bazaar/mod/basket_remove.php', { id : item }, function(data) {
    ...
    refreshSmallBasket : function() {
        jQuery.post('http://bazaar/mod/basket_small_refresh.php', function(data) {
    ...
    add2Basket : function(obj) {
        obj.live('click', function() {
            ...
            jQuery.post('http://bazaar/mod/basket.php', { id : item[0], job : item[1] }, function(data) {
    ...
    updateBasket : function() {
        jQuery.each($('#frm_basket :input'), function() {
            ...
            var val = $(this).val();
            jQuery.post('http://bazaar/mod/basket-qty.php', { id: sid[1], qty: val }, function(data) {
                basketObject.refreshSmallBasket();
                basketObject.refreshBigBasket();
            }, 'html');
        });
    ...
    send2PayPal : function(token) {
        jQuery.post('http://bazaar/mod/paypal.php', { token : token }, function(data) {
            $('#frm_pp').html(data);
            // submit form automatically
            $('#frm_paypal').submit();
        }, 'html');
    },
    emailInactive : function(obj){
        obj.live('click', function(){
            var thisId = $(this).attr('data-id');
            jQuery.getJSON("http://bazaar/mod/resend.php?id="+thisId, function(data){
                if (!data.error) {
                    location.href='http://bazaar/pages/resent.html';
                }else{
                    location.href='http://bazaar/pages/resent-failed.html';
                }
            });
            return false;
        });
