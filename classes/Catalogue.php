...
require_once "Application.php";
	class Catalogue extends Application{
		...
		public function __construct(){
			parent::__construct();
			$this->_path = DS.'media'.DS.'catalogue'.DS;
		}
		...
		public function getProducts($cat){
			$sql = "SELECT * FROM `{$this->_table2}` WHERE `category`='".$this->db->escape($cat)."' ORDER BY `date` DESC";
			return $this->db->fetchAll($sql);
		}

		public function getProduct($id){
			$sql = "SELECT * FROM `{$this->_table2}` WHERE `id`='".$this->db->escape($id)."'";
			return $this->db->fetchOne($sql);
		}
// /////////////
		...
		public function addProduct($params = null){
			if (!empty($params)) {
				...
				$out = $this->db->insert($this->_table2);
		...
		public function updateProduct($params = null, $id = null){
			if (!empty($params) && !empty($id)) {
				...
				return $this->db->update($this->_table2, $id);
			}
		}
		...
		public function addCategory($array = null){
			if (!empty($array) && is_array($array)) {
				$sql = "INSERT INTO `{$this->_table}`
						(`name`, `identity`, `meta_title`, `meta_description`, `meta_keywords`) 
						VALUES ('".$this->db->escape($array['name'])."',
